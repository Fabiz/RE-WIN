...
// require_once 'helpers/Overlay.php'; // replaced by fso: see https://intercom.help/elegantthemes/en/articles/4532734-moving-blog-module-in-child-theme
		...
		if ( have_posts() ) {
			...
			<article id="post-<?php the_ID(); ?>" <?php post_class( 'et_pb_post clearfix' . $no_thumb_class . $overlay_class . $item_class ); ?>>
				...
				<?php } ?>
					...
					$acf_field = get_post_meta($post->ID, 'from', true);
					if ($acf_field) { 
						echo '<div class="from">' . $acf_field . '</div>'; 
					}
					$acf_field = get_post_meta($post->ID, 'to', true);
					if ($acf_field) { 
						echo '<div class="to">' . $acf_field . '</div>'; 
					}
